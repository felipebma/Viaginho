...
public class ServiceDojoAPIService implements HotelAPIServiceInterface {
    ...
    private String getCityId(HotelSearchData hotelSearchData) {
        ...
        ResponseEntity<CitySearchResponse> response = restTemplate.exchange(String.format(url, hotelSearchData.getCity()), HttpMethod.GET, entity,
    ...
    public List<Hotel> getHotels(HotelSearchData hotelSearchData) {
        ...
        HttpEntity<String> entity = new HttpEntity<>(headers);
        ResponseEntity<RapidAPIHotelResponse> response = restTemplate.exchange(String.format(url, cityId, hotelSearchData.getStartDate(), hotelSearchData.getEndDate()), HttpMethod.GET, entity,
        ...
        for(int i = 0; i < response.getBody().getData().getBody().getSearchResults().getResults().size(); i++) {
            ...
            if(result.getRatePlan() == null) continue;
